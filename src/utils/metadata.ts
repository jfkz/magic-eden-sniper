...
export const extendBorsh = () => {
  (BinaryReader.prototype as any).readPubkey = function () {
    ...
    return new solanaWeb3.PublicKey(array);
  };

  (BinaryWriter.prototype as any).writePubkey = function (value: solanaWeb3.PublicKey) {
...
export interface IMetadataExtension {
  ...
  collection: { family: string; name: string; } | string;
}
...
export async function getNft(mint_address: solanaWeb3.PublicKey) {
  try {
    const [metadata_pda] = await solanaWeb3.PublicKey.findProgramAddress([
      Buffer.from('metadata'),
      METADATA_PUBKEY.toBuffer(),
      new solanaWeb3.PublicKey(mint_address).toBuffer(),
    ], METADATA_PUBKEY);

    const [edition_pda] = await solanaWeb3.PublicKey.findProgramAddress([
      Buffer.from('metadata'),
      METADATA_PUBKEY.toBuffer(),
      new solanaWeb3.PublicKey(mint_address).toBuffer(),
      Buffer.from('edition')
    ...
    const nft_metadata: { data: IMetadataExtension } = await axios.get(metadata.data.uri);

    return {
      edition,
      metadata,
      nft_metadata: nft_metadata.data,
    };
